...
set "SOURCE=E:\MegaMelange\unreal-mcp-main\unreal-mcp-main\Python"
set "TARGET=Python"
if not exist "%TARGET%" (
	mkdir "%TARGET%"
)
...
set "SRC=E:\CINEVStudio\CINEVStudio\Plugins\MegaMelange\Source\MegaMelange"
set "DST=MCPGameProject\Plugins\MegaMelange\Source\MegaMelange"

REM Remove the entire MegaMelange Source directory first
if exist "%DST%" (
	echo Removing existing MegaMelange Source directory...
	rmdir /s /q "%DST%"
)

REM Create fresh directory structure
if not exist "MCPGameProject\Plugins\MegaMelange\Source" (
	mkdir "MCPGameProject\Plugins\MegaMelange\Source"
)
mkdir "%DST%"
...
set "SRC=MCPGameProject\Plugins\MegaMelange\Source\MegaMelange"
set "DST=E:\CINEVStudio\CINEVStudio\Plugins\MegaMelange\Source\MegaMelange"
if not exist "%DST%" (
	mkdir "%DST%"
)
del /Q "%DST%\*" 2>nul
...
if exist "%DST%\MCPGameProject\Plugins\MegaMelange" (
	rmdir /s /q "%DST%\MCPGameProject\Plugins\MegaMelange"
)
if not exist "%DST%" (
	mkdir "%DST%"
)
...
echo Exporting MegaMelange Plugin...
mkdir "%DST%\MCPGameProject\Plugins"
robocopy "MCPGameProject\Plugins\MegaMelange" "%DST%\MCPGameProject\Plugins\MegaMelange" /E /XD "Binaries" "Intermediate" ".git" /XF "*.log"
