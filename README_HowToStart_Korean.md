...
> 🎯 **가장 쉬운 방법**: 모든 의존성을 자동으로 설치하려면!

```bash
# 모든 패키지 자동 설치 및 환경 설정
init-install-packages.bat
```

**이 스크립트가 자동으로 해주는 것들:**
- ✅ Python 가상환경 생성
- ✅ 모든 Python 패키지 설치 (FastMCP, Pydantic 등)
- ✅ Node.js 패키지 설치 (Next.js, React 등)  
- ✅ .env 환경 파일 생성
- ✅ 설치 상태 검증

**스크립트 실행 후:**
1. `Python\.env` 파일에서 `ANTHROPIC_API_KEY` 설정
2. 언리얼 엔진 프로젝트 열기
3. `init-ports.bat` 실행
...
> 💡 **쉬운 방법**: 모든 서비스를 한번에 시작하려면 배치 파일을 사용하세요!
...
**자동 스크립트 사용 전 준비사항:**
1. ✅ 언리얼 엔진 프로젝트가 열려있어야 함
2. ✅ Python 가상환경이 설정되어 있어야 함 (`cd Python && uv venv && uv pip install -e .`)
