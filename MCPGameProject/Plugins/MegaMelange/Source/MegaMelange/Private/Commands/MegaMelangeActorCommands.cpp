...
FMegaMelangeActorCommands::FMegaMelangeActorCommands()
{
}

TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
	...
	return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Unknown actor command: %s"), *CommandType));
}

TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleGetActorsInLevel(const TSharedPtr<FJsonObject>& Params)
{
	// Get the world - runtime compatible way
	UWorld* World = GetCurrentWorld();
	if (!World)
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to get world context"));
	}
	...
	for (AActor* Actor : AllActors)
	{
		if (Actor)
		{
			ActorArray.Add(FMegaMelangeCommonUtils::ActorToJson(Actor));
		}
	}
...
TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleFindActorsByName(const TSharedPtr<FJsonObject>& Params)
{
	FString Pattern;
	if (!Params->TryGetStringField(TEXT("pattern"), Pattern))
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'pattern' parameter"));
	}
	...
	for (TActorIterator<AActor> ActorItr(World); ActorItr; ++ActorItr)
	{
		...
		{
			MatchingActors.Add(FMegaMelangeCommonUtils::ActorToJson(Actor));
		}
	}
...
TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleCreateActor(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (bNameExists)
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Actor with name '%s' already exists"), *ActorName));
	}
	...
	return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to create actor"));
}

TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleDeleteActor(const TSharedPtr<FJsonObject>& Params)
{
	FString ActorName;
	if (!Params->TryGetStringField(TEXT("name"), ActorName))
	{
	...
	if (!IsValid(World))
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to get valid world"));
	}
	...
	if (!ActorToDelete)
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Actor not found: %s"), *ActorName));
	}
	...
	if (IsValid(ActorToDelete))
	{
		...
		TSharedPtr<FJsonObject> ActorInfo = FMegaMelangeCommonUtils::ActorToJsonObject(ActorToDelete);
	...
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Failed to delete actor: %s"), *ActorName));
	}
}

TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleSetActorTransform(const TSharedPtr<FJsonObject>& Params)
{
	// Get actor name
	FString ActorName;
	if (!Params->TryGetStringField(TEXT("name"), ActorName))
	...
	if (Params->HasField(TEXT("location")))
	{
		NewTransform.SetLocation(FMegaMelangeCommonUtils::GetVectorFromJson(Params, TEXT("location")));
	}
	if (Params->HasField(TEXT("rotation")))
	{
		NewTransform.SetRotation(FQuat(FMegaMelangeCommonUtils::GetRotatorFromJson(Params, TEXT("rotation"))));
	}
	if (Params->HasField(TEXT("scale")))
	{
		NewTransform.SetScale3D(FMegaMelangeCommonUtils::GetVectorFromJson(Params, TEXT("scale")));
	}
	...
	return FMegaMelangeCommonUtils::ActorToJsonObject(TargetActor, true);
}

TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleGetActorProperties(const TSharedPtr<FJsonObject>& Params)
{
	// Get actor name
	FString ActorName;
	if (!Params->TryGetStringField(TEXT("name"), ActorName))
...
UWorld* FMegaMelangeActorCommands::GetCurrentWorld()
{
	UWorld* World = nullptr;
	if (GEngine && GEngine->GetWorldContexts().Num() > 0)
	{
...
AActor* FMegaMelangeActorCommands::FindActorByClassName(const FString& ClassName)
{
	UWorld* World = GetCurrentWorld();
	if (!IsValid(World))
	{
		FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to get valid world"));
		return nullptr;
	}
...
bool FMegaMelangeActorCommands::GetDoublePropertyValue(AActor* Actor, const FName& PropertyName, float& OutValue)
{
	if (!Actor)
	{
		FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Actor not found"));
		return false;
	}
	UClass* ActorClass = Actor->GetClass();
	if (!ActorClass)
	{
		FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to get actor class"));
		return false;
	}
	FProperty* FoundProperty = ActorClass->FindPropertyByName(PropertyName);
	if (!FoundProperty)
	{
		FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Property not found in %s"), *Actor->GetName()));
		return false;
	}
	...
	{
		FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Property '%s' is not a float"), *FoundProperty->GetName()));
		return false;
	}
...
bool FMegaMelangeActorCommands::UpdateDoubleProperty(AActor* Actor, const FName& PropertyName, float NewValue)
{
	if (!Actor)
	{
		FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Ultra Dynamic Sky actor not found"));
		return false;
	}
	UClass* ActorClass = Actor->GetClass();
...
AActor* FMegaMelangeActorCommands::GetUltraDynamicSkyActor()
{
	const FString SkyClassName = TEXT("Ultra_Dynamic_Sky_C");
	return FindActorByClassName(SkyClassName);
}
AActor* FMegaMelangeActorCommands::GetUltraDynamicWeatherActor()
{
	const FString WeatherClassName = TEXT("Ultra_Dynamic_Weather_C");
	return FindActorByClassName(WeatherClassName);
}
TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleGetUltraDynamicSkyProperties(const TSharedPtr<FJsonObject> &Params)
{
	AActor* SkyActor = GetUltraDynamicSkyActor();
	if (!SkyActor)
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Ultra Dynamic Sky actor not found"));
	}
	...
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to get Ultra Dynamic Sky properties"));
	}
...
TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleSetTimeOfDay(const TSharedPtr<FJsonObject>& Params)
{
	double TimeOfDayValue;
	if (!Params->TryGetNumberField(UDSTODJSONKey, TimeOfDayValue))
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'time_of_day' parameter"));
	}
	if (TimeOfDayValue < 0.0 || TimeOfDayValue > 2400.0)
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Time of day must be between 0 and 2400"));
	}
	...
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to update Ultra Dynamic Sky property"));
	}
	TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
...
TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleSetColorTemperature(const TSharedPtr<FJsonObject>& Params)
{
	double ColorTemperatureValue;
	if (!Params->TryGetNumberField(UDSColorTempJSONKey, ColorTemperatureValue))
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'color_temperature' parameter"));
	}
	if (ColorTemperatureValue < 1500.0 || ColorTemperatureValue > 15000.0)
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Color temperature must be between 1500 and 15000"));
	}
...
TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleGetUltraDynamicWeather(const TSharedPtr<FJsonObject>& Params)
{
	AActor* WeatherActor = GetUltraDynamicWeatherActor();
	if (!WeatherActor)
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Ultra Dynamic Weather actor not found"));
	}
	TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
	ResultObj->SetStringField(TEXT("weather_name"), WeatherActor->GetName());
	return ResultObj;
}
TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleSetCurrentWeatherToRain(const TSharedPtr<FJsonObject>& Params)
{
	AActor* WeatherActor = GetUltraDynamicWeatherActor();
	...
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("SetCurrentWeatherToRain function not found on Ultra Dynamic Weather actor"));
	}
...
AActor* FMegaMelangeActorCommands::GetCesiumGeoreferenceActor()
{
	static const FString ClassName = TEXT("CesiumGeoreference");
	return FindActorByClassName(ClassName);
}

TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleSetCesiumLatitudeLongitude(const TSharedPtr<FJsonObject>& Params)
{
	double Latitude = 0.0;
	...
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'latitude' or 'longitude' parameter"));
	}
	AActor* CesiumActor = GetCesiumGeoreferenceActor();
	if (!CesiumActor)
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Cesium Georeference actor not found"));
	}
	...
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to update Cesium properties"));
	}
...
TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleGetCesiumProperties(const TSharedPtr<FJsonObject>& Params)
{
	AActor* CesiumActor = GetCesiumGeoreferenceActor();
	if (!CesiumActor)
	{
	...
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to get Cesium properties"));
	}
	TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
	ResultObj->SetNumberField(CesiumLatitudeJSONKey, Latitude);
	ResultObj->SetNumberField(CesiumLongitudeJSONKey, Longitude);
...
TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleCreateMMControlLight(const TSharedPtr<FJsonObject> &Params)
{
	// Get required light_name parameter
	FString LightName;
	if (!Params->TryGetStringField(TEXT("light_name"), LightName))
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'light_name' parameter"));
	}
	...
	if (bNameExists)
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Light with name '%s' already exists"), *LightName));
	}
	...
	if (!NewLightActor)
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to spawn light actor"));
	}
...
TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleGetMMControlLights(const TSharedPtr<FJsonObject> &Params)
{
	// Get current world
	UWorld* World = GetCurrentWorld();
	if (!World)
...
TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleUpdateMMControlLight(const TSharedPtr<FJsonObject> &Params)
{
	// Get required light_name parameter
	FString LightName;
	if (!Params->TryGetStringField(TEXT("light_name"), LightName))
	...
	if (!TargetLightActor)
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("MM Control Light not found: %s"), *LightName));
	}
	...
	if (Params->HasField(TEXT("location")))
	{
		FVector NewLocation = FMegaMelangeCommonUtils::GetVectorFromJson(Params, TEXT("location"));
...
TSharedPtr<FJsonObject> FMegaMelangeActorCommands::HandleDeleteMMControlLight(const TSharedPtr<FJsonObject> &Params)
{
	// Get required light_name parameter
	FString LightName;
	if (!Params->TryGetStringField(TEXT("light_name"), LightName))
	...
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Failed to delete MM Control Light: %s"), *LightName));
	}
}