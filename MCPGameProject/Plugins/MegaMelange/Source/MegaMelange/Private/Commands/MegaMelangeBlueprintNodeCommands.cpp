...
DEFINE_LOG_CATEGORY_STATIC(LogMegaMelange, Log, All);

FMegaMelangeBlueprintNodeCommands::FMegaMelangeBlueprintNodeCommands()
{
}

TSharedPtr<FJsonObject> FMegaMelangeBlueprintNodeCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Unknown blueprint node command: %s"), *CommandType));
}

TSharedPtr<FJsonObject> FMegaMelangeBlueprintNodeCommands::HandleConnectBlueprintNodes(const TSharedPtr<FJsonObject>& Params)
{
    ...
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'source_node_id' parameter"));
    }

    FString TargetNodeId;
    if (!Params->TryGetStringField(TEXT("target_node_id"), TargetNodeId))
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'target_node_id' parameter"));
    }

    FString SourcePinName;
    if (!Params->TryGetStringField(TEXT("source_pin"), SourcePinName))
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'source_pin' parameter"));
    }

    FString TargetPinName;
    if (!Params->TryGetStringField(TEXT("target_pin"), TargetPinName))
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'target_pin' parameter"));
    }
    ...
    // Get the event graph
    UEdGraph* EventGraph = FMegaMelangeCommonUtils::FindOrCreateEventGraph(Blueprint);
    if (!EventGraph)
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to get event graph"));
    }
    ...
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Source or target node not found"));
    }

    // Connect the nodes
    if (FMegaMelangeCommonUtils::ConnectGraphNodes(EventGraph, SourceNode, SourcePinName, TargetNode, TargetPinName))
    {
    ...
    return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to connect nodes"));
}

TSharedPtr<FJsonObject> FMegaMelangeBlueprintNodeCommands::HandleCreateInputMapping(const TSharedPtr<FJsonObject>& Params)
{
    // Get required parameters
    FString ActionName;
    if (!Params->TryGetStringField(TEXT("action_name"), ActionName))
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'action_name' parameter"));
    }

    FString Key;
    if (!Params->TryGetStringField(TEXT("key"), Key))
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'key' parameter"));
    }
    ...
    if (!InputSettings)
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to get input settings"));
    }
...
TSharedPtr<FJsonObject> FMegaMelangeBlueprintNodeCommands::HandleAddBlueprintGetSelfComponentReference(const TSharedPtr<FJsonObject>& Params)
{
    // Get required parameters
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    ...
    {
        NodePosition = FMegaMelangeCommonUtils::GetVector2DFromJson(Params, TEXT("node_position"));
    }
    ...
    if (!GetComponentNode)
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to create get component node"));
    }
...
TSharedPtr<FJsonObject> FMegaMelangeBlueprintNodeCommands::HandleAddBlueprintEvent(const TSharedPtr<FJsonObject>& Params)
{
    // Get required parameters
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    ...
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'event_name' parameter"));
    }
    ...
    UK2Node_Event* EventNode = FMegaMelangeCommonUtils::CreateEventNode(EventGraph, EventName, NodePosition);
    if (!EventNode)
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to create event node"));
    }
...
TSharedPtr<FJsonObject> FMegaMelangeBlueprintNodeCommands::HandleAddBlueprintFunctionCall(const TSharedPtr<FJsonObject>& Params)
{
    // Get required parameters
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    ...
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'function_name' parameter"));
    }
    ...
    if (Function && !FunctionNode)
    {
        FunctionNode = FMegaMelangeCommonUtils::CreateFunctionCallNode(EventGraph, Function, NodePosition);
    }
    
    if (!FunctionNode)
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Function not found: %s in target %s"), *FunctionName, Target.IsEmpty() ? TEXT("Blueprint") : *Target));
    }
    ...
    if (Params->HasField(TEXT("params")))
    {
        ...
        if (Params->TryGetObjectField(TEXT("params"), ParamsObj))
        {
            ...
            for (const TPair<FString, TSharedPtr<FJsonValue>>& Param : (*ParamsObj)->Values)
            {
                ...
                UEdGraphPin* ParamPin = FMegaMelangeCommonUtils::FindPin(FunctionNode, ParamName, EGPD_Input);
                if (ParamPin)
                {
                    ...
                    if (ParamValue->Type == EJson::String)
                    {
                        ...
                        if (ParamPin->PinType.PinCategory == UEdGraphSchema_K2::PC_Class)
                        {
                            ...
                            if (!Class)
                            {
                                ...
                                UE_LOG(LogMegaMelange, Display, TEXT("FindObject<UClass> failed. Assuming soft path  path: %s"), *ClassName);
                            }
                            ...
                            if (!Class)
                            {
                                ...
                                UE_LOG(LogMegaMelange, Display, TEXT("Trying Engine module path: %s"), *EngineClassName);
                            }
                            
                            if (!Class)
                            {
                                UE_LOG(LogMegaMelange, Error, TEXT("Failed to find class '%s'. Make sure to use the exact class name with proper prefix (A for actors, U for non-actors)"), *ClassName);
                                return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Failed to find class '%s'"), *ClassName));
                            }

                            const UEdGraphSchema_K2* K2Schema = Cast<const UEdGraphSchema_K2>(EventGraph->GetSchema());
                            if (!K2Schema)
                            {
                                UE_LOG(LogMegaMelange, Error, TEXT("Failed to get K2Schema"));
                                return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to get K2Schema"));
                            }

                            K2Schema->TrySetDefaultObject(*ParamPin, Class);
                            if (ParamPin->DefaultObject != Class)
                            {
                                UE_LOG(LogMegaMelange, Error, TEXT("Failed to set class reference for pin '%s' to '%s'"), *ParamPin->PinName.ToString(), *ClassName);
                                return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Failed to set class reference for pin '%s'"), *ParamPin->PinName.ToString()));
                            }

                            UE_LOG(LogMegaMelange, Log, TEXT("Successfully set class reference for pin '%s' to '%s'"), *ParamPin->PinName.ToString(), *ClassName);
                            continue;
                        }
...
TSharedPtr<FJsonObject> FMegaMelangeBlueprintNodeCommands::HandleAddBlueprintVariable(const TSharedPtr<FJsonObject>& Params)
{
    // Get required parameters
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    ...
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'variable_name' parameter"));
    }

    FString VariableType;
    if (!Params->TryGetStringField(TEXT("variable_type"), VariableType))
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'variable_type' parameter"));
    }
    ...
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Unsupported variable type: %s"), *VariableType));
    }
...
TSharedPtr<FJsonObject> FMegaMelangeBlueprintNodeCommands::HandleAddBlueprintInputActionNode(const TSharedPtr<FJsonObject>& Params)
{
    // Get required parameters
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    ...
    UK2Node_InputAction* InputActionNode = FMegaMelangeCommonUtils::CreateInputActionNode(EventGraph, ActionName, NodePosition);
    if (!InputActionNode)
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to create input action node"));
    }
...
TSharedPtr<FJsonObject> FMegaMelangeBlueprintNodeCommands::HandleAddBlueprintSelfReference(const TSharedPtr<FJsonObject>& Params)
{
    // Get required parameters
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    ...
    UK2Node_Self* SelfNode = FMegaMelangeCommonUtils::CreateSelfReferenceNode(EventGraph, NodePosition);
    if (!SelfNode)
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Failed to create self node"));
    }
...
TSharedPtr<FJsonObject> FMegaMelangeBlueprintNodeCommands::HandleFindBlueprintNodes(const TSharedPtr<FJsonObject>& Params)
{
    // Get required parameters
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    ...
    {
        return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'node_type' parameter"));
    }
    ...
    if (NodeType == TEXT("Event"))
    {
        ...
        if (!Params->TryGetStringField(TEXT("event_name"), EventName))
        {
            return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Missing 'event_name' parameter for Event node search"));
        }
