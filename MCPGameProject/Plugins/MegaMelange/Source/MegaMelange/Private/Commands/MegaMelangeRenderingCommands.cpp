...
FMegaMelangeRenderingCommands::FMegaMelangeRenderingCommands()
{
}

TSharedPtr<FJsonObject> FMegaMelangeRenderingCommands::HandleTakeScreenshot(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("Resolution multiplier must be between 1.0 and 8.0"));
	}
	...
	else
	{
		...
		return FMegaMelangeCommonUtils::CreateErrorResponse(TEXT("No valid world context found"));
	}
...
TSharedPtr<FJsonObject> FMegaMelangeRenderingCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
	if (CommandType == TEXT("take_screenshot"))
	{
		return HandleTakeScreenshot(Params);
	}
	return FMegaMelangeCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Unknown rendering command: %s"), *CommandType));
}
